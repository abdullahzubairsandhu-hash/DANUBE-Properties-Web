...
function BlogModalContent({ isOpen, onClose, onSave, initialData }: BlogModalProps) {
  ...
  const updateTextBlock = (index: number, lang: 'en' | 'ar', val: string) => {
    setFormData(prev => {
      ...
      newBlocks[index] = { ...currentBlock, value: { ...currentValue, [lang]: val } };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="bg-white rounded-xl w-full max-w-5xl max-h-[90vh] overflow-y-auto p-8 shadow-2xl font-medium">
        ...
        <div className="mb-8 p-4 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
           <label className="block text-xs font-black text-gray-400 mb-3 uppercase tracking-widest">Blog Thumbnail</label>
           <div className="flex items-center gap-6">
              {formData.thumbnail && (
                <div className="relative w-32 h-20 rounded-lg overflow-hidden border border-gray-300 shadow-sm bg-white">
                  <CldImage width="128" height="80" src={formData.thumbnail} className="object-cover w-full h-full" alt="Preview" />
                </div>
              )}
              <CldUploadWidget 
        ...
        {/* DATE & EXCERPTS (Condensed for brevity, same logic as News) */}
        <div className="mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div>
            <label className="block text-xs font-bold text-gray-400 mb-1 uppercase">Excerpt (EN)</label>
        ...
        <div className="space-y-6 mb-8">
          <label className="block text-xs font-black text-gray-400 uppercase tracking-widest">Blog Content Sections</label>
          {formData.blocks.map((block, idx) => (
            <div key={idx} className="bg-gray-50 p-6 rounded-lg border border-gray-200 relative group">
              ...
              <div className="flex items-center gap-2 mb-4 text-[#BDA588] font-bold text-xs uppercase">
                {block.type === 'text' ? <><Type size={16}/> Text Block</> : <><ImageIcon size={16}/> Image Block</>}
              </div>
              {block.type === 'text' ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <textarea className={inputClasses} placeholder="English..." rows={4} value={(block.value as {en: string, ar: string}).en} onChange={(e) => updateTextBlock(idx, 'en', e.target.value)} />
                  <textarea dir="rtl" className={`${inputClasses} text-right`} placeholder="عربي..." rows={4} value={(block.value as {en: string, ar: string}).ar} onChange={(e) => updateTextBlock(idx, 'ar', e.target.value)} />
                </div>
              ) : (
                <div className="flex items-center gap-4">
                   <CldUploadWidget uploadPreset="danube_unsigned" onSuccess={(res: CloudinaryUploadWidgetResults) => { if (res.info && typeof res.info !== 'string') updateImageBlock(idx, res.info.public_id); }}>
                     {({ open }) => (
                       <button type="button" onClick={() => open()} className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-lg font-bold text-sm text-black shadow-sm hover:bg-gray-50">
          ...
          <div className="flex gap-4">
            <button type="button" onClick={() => addBlock('text')} className="flex items-center gap-2 px-6 py-3 bg-black text-white rounded-lg hover:bg-[#BDA588] transition-all text-xs font-bold uppercase tracking-widest"><Plus size={16}/> ADD TEXT</button>
...
export default function BlogModal(props: BlogModalProps) {
  return (
    <ClientOnlyAdmin>
      <BlogModalContent key={props.initialData?._id || 'new'} {...props} />
    </ClientOnlyAdmin>
  );
}