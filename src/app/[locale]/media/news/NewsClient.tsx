...
import NewsModal, { INews } from "@/components/admin/NewsModal";
...
export default function NewsClient({ locale }: { locale: string }) {
  ...
  const handleSave = async (formData: INews) => {
    try {
  ...
  return (
    <main className="min-h-screen bg-white pt-20">
      <section className="relative w-full h-[40vh] md:h-[50vh] overflow-hidden">
        <CldImage src="skyline_company-profile_tgvoja" alt="News" fill className="object-cover" priority />
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 py-24">
        ...
        <div className="flex flex-col gap-12">
          {articles.map((article) => (
            <div key={article._id} className="flex flex-col md:flex-row gap-8 items-center border-b border-gray-100 pb-12 group">
              <div className="w-full md:w-1/3 relative aspect-video md:aspect-square lg:aspect-video rounded-sm overflow-hidden shadow-lg">
                <CldImage src={article.thumbnail} alt="thumbnail" fill className="object-cover hover:scale-105 transition-transform duration-700" />
              </div>
              ...
              <div className="w-full md:w-2/3 flex flex-col">
                <div className="flex gap-4 items-center mb-3">
                  <span suppressHydrationWarning className="font-secondary text-gray-400 text-xs md:text-sm uppercase tracking-widest font-medium">
                    {new Date(article.publishedAt).toLocaleDateString(locale, { month: 'long', day: 'numeric', year: 'numeric' })}
                  </span>
                  <span className="h-4 w-[1px] bg-gray-200"></span>
                  <span suppressHydrationWarning className="font-secondary text-gray-400 text-xs md:text-sm font-medium">
                ...
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => { setEditingArticle(article); setIsModalOpen(true); }} className="p-2 text-blue-600 hover:bg-gray-100 rounded-full transition-colors"><Edit2 size={16}/></button>
                    <button onClick={() => handleDelete(article._id!)} className="p-2 text-red-600 hover:bg-gray-100 rounded-full transition-colors"><Trash2 size={16}/></button>
                  </div>
                  )}
                  </ClientOnlyAdmin>
                </div>
                ...
                <p className="font-secondary text-gray-600 text-sm md:text-base mb-6 font-medium line-clamp-2">
                  {article.excerpt[isEn ? 'en' : 'ar']}
                </p>

                <Link href={`/${locale}/media/news/${article.slug}`} className="bg-[#BDA588] text-white px-8 py-3 w-fit uppercase font-secondary tracking-widest text-xs hover:bg-black transition-colors duration-300 shadow-md">
      ...
      <NewsModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSave={handleSave} initialData={editingArticle} />
